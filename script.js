...
$(document).ready(function() {
    $('.marquee').marqueeify({
        speed: 300,
        bumpEdge: function() {
            var hueRotateValue = Math.floor(Math.random() * 360); // Random hue rotation degree
            // Correctly generate a new RGBA color for the overlay
            var newOverlayColor = `rgba(${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, 0.5)`;
            $('.marquee').css('--overlay-color', newOverlayColor);
            // Apply a hue rotation filter to .logo elements
            $('.marquee .logo').css('filter', `hue-rotate(${hueRotateValue}deg)`);
        }
    });
});
